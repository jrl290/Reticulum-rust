...
mod tests {
    ...
    /// Round-trip: sender encrypts with a destination ratchet PUBLIC key,
    /// receiver decrypts via `decrypt_with_ratchets` using the matching
    /// PRIVATE key in the dest_ratchets list.  Mirrors the Meshchat → Retichat
    /// propagation path that is currently failing in production.
    #[test]
    fn test_ratchet_encrypt_decrypt_roundtrip() {
        use rand::Rng;
        use x25519_dalek::{PublicKey as X25519PublicKey, StaticSecret as X25519PrivateKey};

        let mut receiver = Identity::new(true);
        let plaintext = b"propagated payload";

        // Generate a destination-style ratchet keypair (what Destination::rotate_ratchets does)
        let mut ratchet_seed = [0u8; 32];
        rand::thread_rng().fill(&mut ratchet_seed);
        let ratchet_prv = X25519PrivateKey::from(ratchet_seed);
        let ratchet_pub = X25519PublicKey::from(&ratchet_prv);

        // Sender side: encrypt against the receiver identity, using ratchet pub
        let ciphertext = receiver
            .encrypt_with_ratchet(plaintext, Some(ratchet_pub.as_bytes()))
            .expect("ratchet encrypt should succeed");

        // Receiver side: pass ratchet PRIVATE key as one of dest_ratchets
        let dest_ratchets = vec![ratchet_seed.to_vec()];
        let decrypted = receiver
            .decrypt_with_ratchets(&ciphertext, Some(&dest_ratchets))
            .expect("ratchet decrypt should succeed");

        assert_eq!(&decrypted, plaintext, "ratchet round-trip must preserve plaintext");
    }

    /// Same ratchet round-trip but with the matching key buried among many
    /// unrelated decoy ratchets (simulates the 273-ratchet Retichat store).
    #[test]
    fn test_ratchet_decrypt_finds_correct_key_among_many() {
        use rand::Rng;
        use x25519_dalek::{PublicKey as X25519PublicKey, StaticSecret as X25519PrivateKey};

        let mut receiver = Identity::new(true);
        let plaintext = b"needle in haystack";
        ...
        // Build a haystack of 100 decoy keys + the real one in the middle
        let mut dest_ratchets: Vec<Vec<u8>> = Vec::with_capacity(101);
        for _ in 0..50 {
            let mut decoy = [0u8; 32];
            rand::thread_rng().fill(&mut decoy);
            dest_ratchets.push(decoy.to_vec());
        }
        dest_ratchets.push(ratchet_seed.to_vec());
        for _ in 0..50 {
            let mut decoy = [0u8; 32];
            rand::thread_rng().fill(&mut decoy);
            dest_ratchets.push(decoy.to_vec());
        ...
        let decrypted = receiver
            .decrypt_with_ratchets(&ciphertext, Some(&dest_ratchets))
            .expect("decrypt must find correct ratchet among decoys");
